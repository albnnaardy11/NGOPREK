    ...
    return (
        <div className="w-14 flex flex-col items-center py-6 bg-[#050507] border-r border-white/5 z-50">
            ...
            <div className="flex flex-col gap-5 flex-grow">
                {navItems.map((item, i) => (
                    <button 
                        ...
                        title={item.label}
                    >
                        <item.icon className={`w-5 h-5 ${item.active ? 'neon-text-cyan' : 'group-hover:scale-105 transition-transform'}`} />
                        {item.active && (
            ...
            <div className="mt-auto flex flex-col gap-4 items-center">
                <button className="p-2.5 rounded-xl text-gray-600 hover:text-gray-400 hover:bg-white/5 transition-all">
